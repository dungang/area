...
namespace dungang\area\widgets;
...
class ActiveField extends Component
{
    ...
    {
        $this->_attribute = $this->attribute;
    ...
    public function label($label = null, $options = [])
    {
        ...
        if(is_array($this->_attribute)) {
            $for = ArrayHelper::remove($options, 'for', Html::getInputId($this->model, $this->attribute));
            $this->parts['{label}'] = Html::label(\Yii::t('app',Inflector::camel2words($this->attribute)),$for,$options);
        } else {
    ...
    public function error($options = [])
    {
        ...
        if(is_array($this->_attribute)){
            $this->parts['{error}']= self::Errors($this->model, $this->_attribute, $options);
        } else {
     ...
     * @param array $attributes the attribute name or expression. See [[getAttributeName()]] for the format
     ...
     */
    public static function Errors($model, $attributes, $options = [])
    {
        $errors = '';
        ...
        return Html::tag($tag, $encode ? Html::encode($errors) : $errors, $options);
    }
    ...
    protected function getClientAttribute($attribute)
    {
        $attribute = Html::getAttributeName($attribute);
        ...
        if(is_array($this->_attribute)){
            ...
            $containerId = $this->getInputId();
            $inputID = $options['id']= $containerId .$attribute;

        } else {
            $containerId = $inputID = $this->getInputId();
            $options['id'] = Html::getInputId($this->model, $attribute);
        }
        $options['name'] = $attribute;
        $options['container'] = isset($this->selectors['container']) ? $this->selectors['container'] : ".field-$containerId";
    ...
    protected function getClientOptions()
    {
        ...
        if (is_array($this->_attribute)) {
            foreach($this->_attribute as $attribute){
                $atr = $this->getClientAttribute($attribute);
                $opts[] = $atr;
            }
        } else  {
            $opts[] = $this->getClientAttribute($this->_attribute);
        }
        return $opts;
    }
